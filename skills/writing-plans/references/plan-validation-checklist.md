...
This checklist is embedded in the Stage 3 → Stage 4 transition. The `validate-plan-coverage.py` script at `scripts/stage-validators/validate-plan-coverage.py` runs these checks deterministically. Manual checklist use is the fallback when the script is not yet built.
