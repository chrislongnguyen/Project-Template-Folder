...
**Use the hybrid strategy — pick the right tool per search angle:**

**WebSearch (built-in, always available)**
- Best for: recent news, date-sensitive queries, domain-filtered lookups
- Parameters: `query` (required), optional `allowed_domains`, `blocked_domains`
- Example: `WebSearch(query="quantum computing 2025 breakthroughs")`

**Exa MCP tools (when available — preferred for most queries)**
- Exa web search — semantic/neural search, finds conceptually relevant results keyword search misses
- Exa code context search — code examples from GitHub, Stack Overflow, docs
- Exa company research — business intelligence, funding, news
- Exa crawling — full page content extraction (replaces WebFetch for deep dives)

**Routing rules:**
- Conceptual/semantic queries → Exa web search (neural mode)
- Code examples, implementations → Exa code context search
- Company/market research → Exa company research
- Full page content → Exa crawling preferred, WebFetch as fallback
- Breaking news, recent events → WebSearch (fresher index)
- Domain-restricted academic search → WebSearch with `allowed_domains`
- If Exa MCP is not available, fall back to WebSearch + WebFetch for all queries

**IMPORTANT: Never mix parameter styles between tools** — each tool has its own schema. Use the tool names as they appear in your loaded tools list.
...
**Example parallel execution (hybrid — Exa + WebSearch):**
```
[Single message with multiple tool calls — route each angle to the best tool]

# Semantic/conceptual → Exa web search
- Exa web search(query="quantum computing state of the art")
- Exa web search(query="quantum computing limitations and challenges")

# Code/implementation → Exa code context
- Exa code context(query="quantum computing simulation library Python")

# Recent news/events → WebSearch (fresher index)
- WebSearch(query="quantum computing breakthroughs 2025 2026")

# Academic/domain-filtered → WebSearch with domain filter
- WebSearch(query="quantum error correction research", allowed_domains=["arxiv.org", "scholar.google.com"])

# Deep-dive agents → parallel sub-agents
- Task(subagent_type="general-purpose", description="Analyze quantum computing papers", prompt="...")
- Task(subagent_type="general-purpose", description="Industry analysis", prompt="...")
```

**Fallback example (WebSearch only — no Exa available):**
```
...
- WebSearch(query="quantum error correction research", allowed_domains=["arxiv.org"])
- Task(subagent_type="general-purpose", description="Analyze papers", prompt="...")
```
...
**Techniques:**
- Use Exa web search (neural mode) for semantic/conceptual exploration (preferred when available)
- Use Exa code context search for code examples and implementations
- Use Exa company research for business/market intelligence
- Use Exa crawling for full page content extraction
- Use WebSearch for recent news, date-sensitive queries, and domain-filtered academic search
- Use WebFetch as fallback when Exa crawling is unavailable
