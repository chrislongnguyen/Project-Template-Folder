...
1. **MODEL AS SYSTEM:** Identify all 7 components (EPS, Input, EOP, Environment, Tools, Agent, Action) for the system being designed. Never treat a task as isolated — always identify which components are in play and which are missing or misaligned.
...
- **Apply META-RULE 1 (MODEL AS SYSTEM):** Identify all 7 components (EPS, Input, EOP, Environment, Tools, Agent, Action) for the system being designed
...
Use the extended template at `templates/vana-spec-template.md` for the full §0 structure.
...
Use the extended template at `templates/vana-spec-template.md` for the full §6 structure.
