...
1. **Stale path references** — Old skills reference `engine/`, `.cursor/` which no longer exist in the template. Always use current paths: `.claude/skills/`, `_genesis/skills/`, or `{N}-{ZONE}/skills/`. Check the actual repo structure before writing any path.
