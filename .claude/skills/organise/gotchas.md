...
**Symptom:** 2-organised page only answers L1 questions ("what is it") without reaching L2 ("why does it work/fail").
...
**Symptom:** Page exists in 2-organised/ but QMD auto-recall doesn't surface it. Dashboard shows stale data.
**Fix:** Steps 4 and 5 of the pipeline are mandatory. Every organise run must append to _log.md AND update _index.md. Check both after every run.
...
**EP:** EP-07 (Amnesia-First) — the agent doesn't remember what's already organised; _index.md is the memory.

## G5: Losing source attribution

**Symptom:** Organised page has no `source` frontmatter or Sources section.
**Fix:** Every page must have `source:` in frontmatter pointing to the `1-captured/` file, and a Sources section with `[[links]]` to the original.
**EP:** EP-12 (Verified Handoff) — traceability from output to input.

## G6: Large source treated as small (context overflow)

**Symptom:** Agent reads entire 31K-line file into context, output degrades mid-organise (hallucinated sections, missing later content, truncated pages).
...
## G7: Cross-section links missing in multi-page organise

**Symptom:** Pages from same source don't link to each other. Graph is disconnected.
**Root cause:** Each page written independently without checking what other pages were created in the same organise run.
...
**Root cause:** Agent treats `1-captured/` as working space instead of immutable archive.
**Fix:** `1-captured/` is IMMUTABLE. Never edit, rename, or delete files there. All transformation happens in `2-organised/`. If a source needs cleanup, note it in the page's Sources section, don't change the original.
**EP:** EP-01 (Brake Before Gas) — preserve the original before transforming.

## G9: Version/status not set on new pages

**Symptom:** New organised page has no version or status in frontmatter, or version is "0.1".
...
## G10: QMD not re-embedded after organise

**Symptom:** New pages exist in organised/ but QMD semantic search doesn't find them.
**Root cause:** QMD needs `qmd embed` after new files are added to update vector index.
**Fix:** After organise, remind user to run `qmd embed` (or it runs automatically on next session start if configured). The post-validation `pkb-lint.sh` check will warn about unembedded files in future versions.
**EP:** EP-14 (Script-First Delegation) — embedding is a deterministic operation.
**Post-organise command:** `qmd update 2-organised && qmd embed` — add to Validation Gate if QMD 2-organised collection is configured.
