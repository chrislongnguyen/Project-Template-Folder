...
1. **CHECK WORKSTREAM:** Which ALPEI workstream is this task in? Run `/dsbv status` to see current progress. If ambiguous, ask the user.
...
7. **CHECK VERSION CONSISTENCY:** Verify that the workstream you are about to modify has version metadata consistent with its DSBV phase. Cross-workstream versions must not regress (e.g., a PLAN artifact should not reference an outdated ALIGN version).
