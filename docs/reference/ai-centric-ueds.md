...
│
└── ALIGNMENT DRIFT (agent optimizes for proxy metrics, not UDO)
    Definition: Agent chases measurable goals that don't actually serve the UDO
    ...
    Impact: RACI must include validation that AI actions still serve UDO
```
...
- Alignment Drift → Explicit objective monitoring (is the AI's path still serving the UDO?)
...
- Authority to decide what the actual UDO is (others may want something else)
...
**Human-Centric Summary (WES 4 — HIGH):**
Designers suffer complexity bias (underestimate system interactions) and familiarity bias (default to what they know). UBS/UDS mapping from Phase B → EPS → UES → EOP progression skipped.

**AI-Centric Translation:**

AI designs the solution by systematically working through UBS/UDS → EPS → UES → EOP. Complexity is made visible; novel approaches are generated.
...
PROCESS:    1. AI agent extracts domain UBS/UDS from Phase B (scope-specific layer)
            2. AI agent generates EPS (principles to disable UBS; enable UDS)
            3. AI agent generates UES.Environment (conditions for EPS to work)
            4. AI agent generates UES.Tools (instruments for EPS to work)
            5. AI agent generates EOP (step-by-step procedure; gated at each step)
            6. AI agent maps interdependencies (which EPS depends on which UES?)
...
PHASES ELIMINATED:
  ...
  ✗ "Let's just start coding" (skipping design; AI enforces EPS → UES → EOP)

PHASES ADDED:
  ✓ Scope-specific UBS/UDS extraction
  ✓ Principle generation (EPS) from UBS/UDS
  ✓ Environment design (UES.Env)
  ✓ Tool selection (UES.Tools)
...
| Resource | Requirement | Why |
|----------|-------------|-----|
| **Capital** | Moderate (~$1–3) | EPS generation, multiple design options, dependency mapping |
| **Capability** | UBS/UDS application; EPS generation; systems design; dependency analysis; EOP writing | Core + specialized prompting |
| **Capacity** | ~15–30K tokens (UBS/UDS analysis, EPS options, UES.Env, UES.Tools, EOP, dependency map) | Large output context |
...
- Experience with similar systems (to validate EPS and UES choices)
...
| **Function** | Extract scope-specific UBS/UDS; generate EPS; design UES.Env + UES.Tools; write EOP; map dependencies |
...
| **Optimal Effectiveness** | 2+ design options presented (different architectural approaches); dependencies fully mapped; EOP includes risk gates; EPS visibly traces to UBS/UDS |

**RACI+ Shift:**

| Role | Human-Centric | AI-Centric |
|------|--------------|------------|
| **R (DOs)** | Developer: designs UES + EOP from UBS/UDS; documents design; maps dependencies | **AI Agent: generates EPS from UBS/UDS; designs UES.Env + UES.Tools; writes EOP; maps all dependencies** |
| **R (DON'Ts)** | Developer: do NOT skip UBS/UDS application; do NOT skip dependency mapping | **AI Agent: do NOT generate EPS without UBS/UDS traceability; do NOT omit EOP risk gates** |
| **A (DOs)** | Product Owner: reviews design; validates vs. scope; approves build plan | **Human Director: reviews design completeness; validates EPS traces to UBS/UDS; validates UES feasible; approves EOP gates** |
...
**Workstream Integration Contract (D24, D28):** Each workstream/book above must define its INPUT (from upstream WS), OUTPUT (to downstream WS), RACI (R = AI Agent role; A = Human Director focus per MCM Table 4b), and UDO before T0 table generation. The integration chain (SYS-A OUTPUT → SYS-B INPUT) creates the fabric connecting all books into one OE System. See MCM §2.7 for rules.
...
Stage E.2 (Design):
  ...
  - EPS generation (principles from UBS/UDS)
...
Stage E.2 (Design)            ~15–30K    (UBS/UDS + EPS + UES + EOP + dependencies)
...
2. **Keeping A (Accountable) with Human:** The Director retains judgment about what to build (UDO), whether the result is good enough (final approval), and whether to proceed (gates at each stage). The human still owns outcomes and consequences.
