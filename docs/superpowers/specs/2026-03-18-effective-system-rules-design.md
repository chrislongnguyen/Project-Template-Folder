...
LTC members at all SFIA levels have access to distilled, actionable reference docs that they load by choice when:
...
### 3.1 `rules/agent-system.md` (~350 lines)
...
#### Section 1: Purpose & Three Principles (~40 lines)
...
#### Section 2: The 8 LLM Fundamental Truths (~80 lines)
...
#### Section 3: The Two Operators (~50 lines)
...
#### Section 4: The 7-Component System (~120 lines)
...
- **"Map Anything New" table** (from Session 1):

| Buzzword | What It Actually Is | Component |
|---|---|---|
| RAG | Retrieves documents to feed agent | Input |
| Prompt engineering | Crafting better task descriptions | Input |
| Context engineering | Designing what info reaches agent | Input + EPS + Environment |
| Fine-tuning | Retraining the model | Agent |
| MCP servers | External tool connections | Tools |
| SKILL.md / .mdc | Reusable procedure files | EOP |
| Hooks / git hooks | Deterministic pre/post-action scripts | Environment |
| CLAUDE.md / .cursorrules | Persistent rules | EPS |
| Chain-of-thought | Visible reasoning steps | Action (observe to debug) |

#### Section 5: Notation (~20 lines)
- Abbreviation table: 7-CS, EPS, EOP, UBS, UDS, LT-N, UT#N, RACI, VANA, A.C., UDO

**What's dropped from sources:** Kitchen Analogy, SFIA Mastery Ladder, Strategy Brief examples, citations/citation index, research questions, research directives, loading protocol, epistemic tags, practice paths, changelog.
...
#### Section 1: Purpose & Foundation (~30 lines)
- What this doc is and when to load it
...
- Relationship to `agent-system.md`: the 7-CS is a specialization where User→Agent, Principles→EPS+EOP, Action separates from process design

#### Section 2: The Universal System Model (~40 lines)
...
#### Section 3: RACI — Who Does What (~25 lines)
...
- When R = AI Agent: behavioral boundaries (Always/Ask/Never) become part of the RACI contract
- Per-step RACI refines system-level RACI in EOP
- UBS/UDS analysis must cover both: UBS(R), UBS(A), UDS(R), UDS(A)

#### Section 4: Force Analysis — UBS & UDS (~50 lines)
- UT#2: Every system has forces blocking it (UBS) and driving it (UDS)
- Role-aware: UBS(R), UBS(A), UDS(R), UDS(A)
- Recursive notation: UBS.UB (what strengthens the blocker), UBS.UD (what weakens the blocker), UDS.UD (what strengthens the driver), UDS.UB (what weakens the driver)
- Why both sides have internal contradictions
- Derisk first: disable UBS before amplifying UDS

#### Section 5: Effective Principles & Standards (~30 lines)
...
#### Section 6: System Boundaries — Input & Output (~60 lines)
...
#### Section 7: The ESD Methodology (~40 lines)
...
#### Section 8: The Value Chain (~20 lines)
...
#### Section 9: Notation (~15 lines)
- Abbreviation table: UT#N, UDO, UBS, UDS, EPS, EOP, UES, ESD, ELF, VANA, RACI, A.C., MECE, OE, UE
...
#### Section 2: The Blame Diagnostic (~20 lines)
...
#### Section 3: Symptom → Root Component Table (~30 lines)
...
| Output is correct but Director rejects it | Human UBS — Director's biases overriding evaluation | Run Force Map (§5). Is System 1 dominant? |
| Rushed delegation, compound errors | Derisk step skipped | Run Derisk Checklist (§4) before re-delegating |
| Inconsistent behavior across sessions | EPS missing or too thin; LT-6 (no memory) | Is CLAUDE.md loaded? Does it cover this case? |

#### Section 4: The Derisk Checklist (~15 lines)
...
#### Section 5: The Force Map (~15 lines)
...
#### Section 6: Automated Diagnostic Integration Points (~30 lines)
Where future diagnostic automation can hook in:

**Post-action hooks:**
- Validate output against AC eval spec (Layer 3 from general-system.md)
- Check output schema against contract fields (Layer 2)
- Log: action type, component trace, pass/fail, tokens consumed

**Per-step monitoring:**
- Check failure mode triggers (Layer 4)
...
#### Section 7: Notation (~10 lines)
- Abbreviation table (subset of agent-system.md notation)

---

### 3.4 CLAUDE.md Updates (~15-20 lines added)

Three new sections added to the template CLAUDE.md between existing sections:
...
- **Building the diagnostic agent/hook** — §6 of agent-diagnostic.md lays the foundation; implementation is a separate task
...
**File header convention:** Each new rule file must begin with the standard header consistent with existing files in `rules/`: title, source-of-truth reference, last-synced date.

**Line count tolerance:** Estimates are +/-15%. Clarity and completeness take priority over hitting exact line targets.

**Cross-reference convention:** Use `(see agent-system.md §2)` format for section references across the three docs, so practitioners and future agents can follow links consistently.

**Extensibility note:** The "Map Anything New" table in agent-system.md is a living artifact — designed to be extended as new AI concepts emerge.
...
2. Session 1 Wiki (Notion, v3, Mar 12) — most current 7-Component System, Map Anything New
...
|---|---|---|---|
| `rules/agent-system.md` | NEW | ~350 | Agent 7-CS: 3 principles, 8 LLM Truths, two operators, 7 components, Map Anything New |
...
| `CLAUDE.md` | EDIT | +15 lines | Summary sections pointing to the three rule files |

**Total new content:** ~825 lines across 3 new files + 15-line CLAUDE.md edit.
