...
def check_traceability(content: str) -> tuple[bool, str]:
    ...
    Looks for UDO reference AND a VANA element reference in the spec.
    """
    has_udo = bool(re.search(r"\bUDO\b", content))
    has_vana = bool(
    ...
    missing = []
    if not has_udo:
        missing.append("UDO (User-Desired Outcome) reference missing")
    if not has_vana:
    ...
    return True, "Traceability chain references present (UDO → VANA → AC-TEST-MAP)"
