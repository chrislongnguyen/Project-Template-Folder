#!/usr/bin/env python3
"""
Stage 4→5 Validator: .exec/ readiness checks — 10-point automated gate.

Standalone implementation; does NOT import from or subprocess-delegate to
...
def parse_dependencies(content: str) -> dict[str, list[str]]:
    ...
    deps: dict[str, list[str]] = {"blocked_by": [], "blocks": []}
    in_deps = False
...
def check_6_status_sync(exec_dir: Path, task_files: list[Path]) -> tuple[bool, str]:
    ...
    for d_val in status.get("deliverables", {}).values():
...
def run_checks(exec_dir: Path) -> int:
    ...
    checks = [
        ...
        ("Check 5: Environment prereqs",         lambda: check_5_environment(task_files)),
