...
1. **Phase 1 — Problem Discovery**: Reconstruct the User's system from ELF output. Who is the User? What blocks them? What drives them? *(Source: ELF EO, User Roles, UBS, UDS)*
...
Layers in order: `EO → User Roles → UBS (+ UBS.UB + UBS.UD) → UDS (+ UDS.UD + UDS.UB) → EP → EOE/EOT → EOP`
...
The documentation format for each project or feature built by the agent. Captures: Document Control, Desired Outcomes, Users & Roles, Effective Operating Principles (UBS/UDS/EP), Environment, Tools, and EOP. One filled wiki per system built. **The wiki is for documenting, not designing.**
...
**EOE/EOT — Effective Operating Environment / Effective Operating Tools**
The concrete environment and tools that implement the EP principles. Organized in 3 universal causal layers. The layer **names** are subject-specific; the **logic** is always the same:
...
In ESD, EOE/EOT becomes the Noun and Noun A.C.s in Phase 3.
...
| EOE/EOT — Layer 1 (Foundational) | Phase 2 → Phase 3 | Noun Layer 1 A.C.s | Foundational components → Noun-ACn (under Layer 1 header) |
| EOE/EOT — Layer 2 (Operational) | Phase 2 → Phase 3 | Noun Layer 2 A.C.s | Operational components → Noun-ACn (under Layer 2 header) |
| EOE/EOT — Layer 3 (Enhancement) | Phase 2 → Phase 3 | Noun Layer 3 A.C.s | Enhancement components → Noun-ACn (under Layer 3 header) |
...
|------|-------|------------------------|
| **R (Responsible)** | [Who does the work — e.g., AI Agent role] | UBS(R): what blocks R from executing; UDS(R): what drives R to succeed |
...
| **C (Consulted)** | [Domain expert / upstream provider] | — |
| **I (Informed)** | [Downstream consumer / team / logs] | — |
...
Derived from ELF UDS layer and all recursive sub-layers. **Role-aware per D25**: UDS must be analyzed from both R and A perspectives established in §3.1 RACI. Row labels use role tags: `UDS(R)`, `UDS(A)`, etc.
...
*All UDS elements discovered here become candidates for the EOP Decision Matrix (§4.4) OPTIMIZE category.*

---

### 3.4 The Blocking System (UBS)

Derived from ELF UBS layer and all recursive sub-layers. **Role-aware per D25**: UBS must be analyzed from both R and A perspectives established in §3.1 RACI. Row labels use role tags: `UBS(R)`, `UBS(A)`, etc.

| Element | Description |
|---|---|
| **UBS(R)** | Root blocking forces from R (Responsible) perspective — what blocks R from executing |
| **UBS(A)** | Root blocking forces from A (Accountable) perspective — what blocks A from ensuring quality |
| **UBS(R).UD** | What drives UBS(R) harder — makes R's blocker stronger |
| **UBS(A).UB** | What disables UBS(A) — the force working in User's favour against A's blocker |
| *...recursive* | Continue for all layers documented in the ELF Learning Book, role-tagged |

*All UBS elements discovered here become candidates for the EOP Decision Matrix (§4.4) DERISK category.*
...
Format: `P[n](S)(R/A/both): [Principle] — Disables [UBS element] / Enables [UDS element]`

**Efficiency Principles (E)**
Govern fast, lean, frugal operation. Primarily address UDS and its recursive sub-layers.
Format: `P[n](E)(R/A/both): [Principle] — Enables [UDS.UD element]`

**Scalability Principles (Sc)**
Govern repeatable, comparable, growth-capable operation.
Format: `P[n](Sc)(R/A/both): [Principle] — Enables [UDS.UD element at recursive depth]`

*The role tag `(R)`, `(A)`, or `(both)` indicates which RACI actor this principle primarily governs. Principles tagged `(R)` translate to Agent constraints; `(A)` translate to Human Director constraints.*

---

### 4.2 Environment (Where)

Derived from ELF EOE (Environment sub-layer). The context where the UE system must operate. Specifies constraints on the operating environment — not the tools themselves (those are in §4.3).
...
Derived from ELF EOT (Tools sub-layer). Organized by EOT 3 causal layers. Layer names are subject-specific — define them before designing. Every tool traces to at least one Principle from §4.1.
...
**Stage 2 — OPTIMIZE (Efficiency + Scalability)**: Steps that amplify the targeted UDS elements — maximise desired output. Only begin after all DERISK steps are gated.
...
*Noun A.C.s: organized by EOE/EOT causal layer (Foundational → Operational → Enhancement) — NOT by S/E/Sc.*
...
*Traceability: EOE/EOT ([Layer1].n, [Layer2].n, [Layer3].n) ← ELF EOE/EOT layer ← EP Principles*
...
| EOE/EOT (3 causal layers) | → Noun A.C.s (by layer) | One EOE/EOT component = one Noun A.C. |
...
*Traceability: EP P1(S) + P2(S) → safety/correctness qualities the Agent must possess*
