...
To ensure effectiveness and risk management, each work stream follows four agile stages. The output of each stage is the input for the next (Scope → Design → Production → Audit).

- **Scope.** Define what needs to be done and why.
- **Design.** Design how to do it.
- **Production.** Do it.
- **Audit.** Verify it was done correctly and feeds the right output forward.
...
**Principle 6: Stages chain forward within a work stream.** The output of each stage is the input for the next: Scope → Design → Production → Audit.
...
|---|---|---|---|---|
|1|Scope|Define what and why|Context, needs, objectives from prior work stream|Objectives and success criteria for this work stream|
...
|4|Audit|Verify correctness|Completed deliverable, original objectives|Validated deliverable ready to feed the next work stream|
...
|STAGES CHAIN FORWARD WITHIN A WORK STREAM|Scope → Design → Production → Audit|Sequential, output-to-input|
