...
source: "Adapted from Vinh branch (Vinh-ALPEI-AI-Operating-System-with-Obsidian)"
---
...
| **VERSION** | **DEPTH** | **DELIVERABLES** | **KEY DIFFERENCE** |
|---|---|---|---|
| **Logic Scaffold** | Rough scope only | Iteration Scope (draft), Task list (conceptual), Dependency map (initial) | High-level structure; no time estimates, no resource assignments |
| **Concept** | Risk-aware planning | Iteration Scope (risk-assessed), Task breakdown (safety-sequenced), UBS Risk Register (populated) | Plans must address sustainability risks first; risk mitigation explicit |
...
- `_genesis/frameworks/` — UES Versioning and VANA criteria (see `UES_VERSION_BEHAVIORS.md` companion docs)
- `_genesis/reference/` — Vinh ALPEI PDFs: Overview, By Work Streams, By Sub-system, Process Requirements, UES Versioning
