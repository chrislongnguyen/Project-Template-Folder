...
**What this doc is:** Canonical reference for configuring an AI agent's 7-CS. Defines the 8 structural limits of LLM models, the two operators, and the 7 components that compensate for those limits.
...
**Mechanism:** All state must be externally provided via Input, EPS, or Tools. Nothing carries between sessions without external storage.
**Compensated by:** Input (load session state/history), Tools (memory vault systems), EPS (rules for what to load at session start).
...
| Programmatic enforcement | Rules loaded via EPS applied consistently, not selectively |
...
```
                    ┌─────────┐
                    │   EPS   │ ← Constitution (constrains ALL below)
                    └────┬────┘
                         │ governs
              ┌──────────┼──────────┐
              ▼          ▼          ▼
         ...
         └────┬────┘ └────┬────┘   │
              ...
              ▼          ▼          │
         ┌─────────────────────┐   │
         │     AGENT           │◄──┘ (EPS constrains Agent directly)
         │  (executes within   │
         │   Environment +     │
         │   Tools)            │
         └────────┬────────────┘
                  ...
                  ▼
         ┌──────────────┐
         │    ACTION     │ ← Emergent (observe, do not configure)
         └──────┬───────┘
                │ yields
                ▼
         ┌──────────────┐
...
Lateral dependencies:
  Environment ──sets hard ceilings for──→ Agent, Tools
  Tools ──extend capabilities of──→ Agent
  Agent ──operates within──→ Environment
```
...
| --- | --------------- | ----------------------------------------------- | ---------------------------------------------- | -------- |
| 1 | **EPS** | Persistent rules — always active, all tasks | CLAUDE.md, .cursorrules, GEMINI.md | 1st |
...
| 4 | **Environment** | Workspace — config, permissions, limits | IDE, terminal, context window, hooks, sandbox | 4th |
...
| 7 | **Action** | Observable execution (emergent — diagnose only) | Reasoning chains, tool calls, file edits | 7th |
...
**System role:** Constrains all other components. No component can violate EPS.
...
> **Always/Ask/Never pattern:** When configuring EPS, define behavioral boundaries: what the agent always does (safe actions, no approval needed), what requires Human Director approval (high-impact actions), and what is prohibited (hard stops, violation = immediate halt).
...
- Contradicts EPS — Agent resolves conflict incorrectly
**Drive output:**
...
**System role:** Sets hard ceilings for Agent and Tools. No component can exceed what Environment allows.
...
- Context window misunderstood — Input + EPS + EOP exceed effective capacity
- Compute insufficient — Agent shortcuts reasoning (amplifies LT-3)
**Drive output:**
- Context budget explicitly allocated: EPS X tokens, Input Y, EOP Z, reasoning the remainder
- Sandboxing that prevents destructive operations while allowing productive ones
- Environment matches task complexity
...
**System role:** Produces the Outcome. When Action fails, root cause is in the other 6 components.
...
- Misdiagnosed — blame on Action instead of tracing to root component
- Over-monitored — excessive logging consumes context budget (LT-7)
- Symptoms treated instead of root causes — failures recur
**Drive output:**
- Trace failures back to the 6 input components via the blame diagnostic: EPS, Input, EOP, Environment, Tools, Agent (in that order)
- Log key decision points for post-mortem analysis
- Design monitoring that catches failures before they reach Outcome
