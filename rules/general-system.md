...
**Relationship to agent-system.md:** The 7-Component System in agent-system.md (see agent-system.md §5) is a specialization of this universal model where User becomes Agent, Principles splits into EPS (always-active) and EOP (on-demand), and Action separates from process design.

---

## 3. The Universal System Model

From UT#1 and the system formula, every system is composed of six components. The table below defines each, its role, and what happens when it is absent.
...
- `UBS.UB` — what weakens the blocker (works IN the User's favor). These are natural allies; Principles should amplify them.
- `UBS.UD` — what strengthens the blocker (works AGAINST the User). These are force multipliers for the blocker; Principles must neutralize them.
...
- `UDS.UB` — what weakens the driver (works AGAINST the User). These are the silent killers of momentum; Principles must disable them.
